...
DATASET_DIR = os.path.join(BASE_DIR, "..", "app", "datasets")
OUTPUT_DIR = os.path.join(BASE_DIR, "..", "app", "datasets_and_models_output", "spark")
...
def coletar_caracteristicas_spark():
    from pyspark.sql import SparkSession
    spark = SparkSession.builder.getOrCreate()
    conf = spark.sparkContext.getConf()
    master = conf.get("spark.master", "unknown")
    is_local = master.startswith("local")

    # Tenta estimar a memória disponível
    executor_memory = conf.get("spark.executor.memory", None)
    if executor_memory is None and is_local:
        total_memory_gb = psutil.virtual_memory().total / (1024 ** 3)
        executor_memory = f"{int(total_memory_gb)}g"

    return {
        "spark_master": master,
        "spark_executor_memory": executor_memory or "unknown",
        "spark_driver_memory": conf.get("spark.driver.memory", "unknown"),
        "spark_executor_cores": conf.get("spark.executor.cores", str(psutil.cpu_count(logical=True)) if is_local else "unknown"),
        "spark_executor_instances": "1" if is_local else conf.get("spark.executor.instances", "unknown"),
        "spark_modo_distribuido": not is_local,
        "spark_modo": "single_node" if is_local else "cluster"
    }

"""
Treina um modelo linear para predição de falhas baseado em uso de memória.
Usa regressão logística com regularização para melhor generalização.
"""
...
def train_validate_save():
    """Treina, valida e salva o modelo de classificação linear."""
    ...
    print("\n===== VALIDAÇÃO CRUZADA DO CLASSIFICADOR LINEAR =====")
    ...
    linear_classifier.fit(X, y_class)
    
    # Avaliar no conjunto de treino
    y_pred = linear_classifier.predict(X)
    y_prob = linear_classifier.predict_proba(X)[:, 1]
    ...
    joblib.dump(linear_classifier, os.path.join(MODELS_OUT_DIR, "linear_memory_classifier.pkl"), compress=3)
    joblib.dump(memory_threshold, os.path.join(MODELS_OUT_DIR, "linear_memory_threshold.pkl"), compress=3)
    joblib.dump(X.columns.tolist(), MEMORY_FEATURES_OUT, compress=3)
    
    # Salvar métricas
    metrics = {
        'linear_memory_classifier': {
    ...
    joblib.dump(metrics, os.path.join(MODELS_OUT_DIR, "linear_memory_metrics.pkl"), compress=3)
    ...
    if cat_cols:
        ohe = linear_classifier.named_steps['preprocessor'].named_transformers_['cat']
    ...
    # Obter coeficientes
    coefficients = linear_classifier.named_steps['classifier'].coef_[0]
